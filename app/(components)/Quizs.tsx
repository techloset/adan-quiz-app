          ...
          {Quiz?.map((item: QuizProps, index: number) => {
            ...
            return (
              <div key={index} className="flex justify-center cursor-pointer">
                <div   onMouseEnter={() => setHover(index)}  className={`self-center py-11 px-[30px] shadow-2xl dark:shadow-blue-900/50   space-y-11 max-w-[352px] flex flex-col rounded-2xl items-center ${ Hovered ? `bg-exact-purple` : "bg-exact-white dark:bg-[#00113B]" }`} >
                  <div className={`p-8 shadow-xl  rounded-full  shadow-${  item.color} ${ Hovered ? "bg-exact-white shadow-gray-800/50 "  : `bg-exact-purple` }`} >
                    <div className={`${ Hovered ? `fill-exact-purple` : "fill-exact-white"} w-[33px] h-[33px]`}>
                  ...
                  <div   className={`dark:bg-[#020E2D] p-[17px] rounded-full hover:relative hover:left-1 hover:bg-[#F3F4F6] dark:fill-blue-600 fill-blue-600 ${ Hovered  ? `bg-exact-white fill-exact-purple dark:bg-exact-white/100 `  : "bg-[#F3F4F6]"} `} >
                    <svg
